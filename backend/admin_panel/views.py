...
from .serializers import (AdminCategoryOfferSerializer, AdminOrderSerializer,
                          ...
                          ChartRevenueSerializer, ProductReportSerializer,
                          SalesReportSerializer, VendorPerformanceSerializer)
...
class AdminStatsAPIView(APIView):
    ...
    def get(self, request):
        ...
        total_revenue = (
            CartOrder.objects.filter(payment_status="paid").aggregate(
                total=Sum("total")
            )["total"]
            or 0
        )
...
class AdminRevenueChartAPIView(APIView):
    ...
    def get(self, request):
        ...
        revenue_data = (
            CartOrder.objects.filter(payment_status="paid", date__gte=start_date)
            .annotate(period_date=trunc_func("date"))
            .values("period_date")
            .annotate(revenue=Sum("total"))
...
class AdminCategoryOfferListCreateAPIView(generics.ListCreateAPIView):
    ...
    queryset = (
        CategoryOffer.objects.all().select_related("category").order_by("-start_date")
    )
    ...
    def get_vendor_performance(self):
        ...
        vendor_performance = (
            CartOrderItem.objects.filter(
                order__payment_status="paid", vendor__isnull=False
            )
            .values("vendor__id", "vendor__name")
            .annotate(
                total_revenue=Sum("total"), total_orders=Count("order", distinct=True)
            )
            ...
            data.append(
                {
                    ...
                    "vendor_name": item[
                        "vendor__name"
                    ],  # Note: vendor__name might need to be vendor__name (from values)
    ...
    def get_product_report(self):
        ...
        products = (
            Product.objects.annotate(
                total_orders=Count(
                    "order_item", filter=Q(order_item__order__payment_status="paid")
                ),
                total_revenue=Sum(
                    "order_item__total",
...
class AdminNotificationListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Notification.objects.filter(user__is_staff=True).order_by("-date")
...
class BestSellingProductsAPIView(APIView):
    ...
    def get(self, request):
        ...
        best_products = (
            CartOrderItem.objects.filter(
                order__payment_status="paid", order__date__gte=start_date
            )
            .values(
                "product__id",
                "product__title",
    ...
    def get(self, request):
        ...
        for item in best_categories:
            ...
            if item["product__category__image"]:
                image_url = request.build_absolute_uri(
                    "/media/" + str(item["product__category__image"])
                )
