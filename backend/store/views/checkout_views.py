...
def send_notification(user=None, vendor=None, order=None, order_item=None):
    ...
    try:
        ...
        logger.info(
            f"Notification created for order {order.id if order else 'unknown'}"
        )
...
def get_paypal_access_token(client_id, secret_id):
    ...
    try:
        response = requests.post(
            token_url, headers=headers, data=data, auth=(client_id, secret_id)
        )
...
def send_all_notifications(order, order_items):
    ...
    for vendor_id, items in vendor_groups.items():
        ...
        try:
            ...
            send_email(
                subject, text_body, html_body, settings.DEFAULT_FROM_EMAIL, vendor_email
            )
        except Exception as e:
...
def deduct_stock(order_items):
    ...
    for item in order_items:
        ...
        if product.stock_qty >= item.qty:
            ...
            logger.warning(
                f"Insufficient stock for {product.title} during payment success."
            )
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            if order.payment_status == "paid":
                ...
                return Response(
                    {"message": "Already paid", "icon": "warn"},
        ...
        try:
            ...
            client.set_app_details(
                {
                    "title": config("APP_TITLE", "Django"),
                    "version": config("APP_VERSION", "4.2"),
                }
            )
...
class PaymentSuccessView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        logger.debug(
            f"Processing payment for order_id: {order_id}, session_id: {session_id}, capture_id: {capture_id}"
        )
        if not order_id:
            logger.warning("Missing order_id in payload")
            return Response(
                {"message": "Missing order_id"}, status=status.HTTP_400_BAD_REQUEST
            )
        try:
            order = CartOrder.objects.get(oid=order_id)
        ...
        if capture_id:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.info(
                        f"PayPal status details: {paypal_data.get('status_details')}"
                    )
                    is_sandbox = "sandbox" in paypal_api_url
                    if status_val == "COMPLETED" or (
                        status_val == "PENDING" and is_sandbox
                    ):
                        with transaction.atomic():
                            locked_order = CartOrder.objects.select_for_update().get(
                                oid=order_id
                            )
                            if locked_order.payment_status == "paid":
                                logger.warning(
                                    f"Order {order_id} already paid (locked check)"
                                )
                                return Response(
                            ...
                            if locked_order.buyer and locked_order.coupons.exists():
                                for coupon in locked_order.coupons.all():
                                    ...
                                    logger.info(
                                        f"Marked coupon {coupon.code} as used by user {locked_order.buyer.id}"
                                    )

                            locked_order.save()
                            deactivate_cart(
                                locked_order.oid,
                                locked_order.buyer_id if locked_order.buyer else None,
                            )
                        ...
                        return Response(
                            {"message": "payment_successful"}, status=status.HTTP_200_OK
                        )
                        ...
                        with transaction.atomic():
                            ...
                            if locked_order.payment_status == "processing":
                                logger.info(
                                    f"Order {order_id} already processing (locked check)"
                                )
                                return Response(
                                    {"message": f"Payment is {status_val.lower()}"},
                            ...
                            locked_order.save()
                        logger.info(
                            f"PayPal payment {status_val.lower()} for order {order_id}"
                        )
                        return Response(
                            {"message": f"Payment is {status_val.lower()}"},
                            status=status.HTTP_202_ACCEPTED,
                        ...
                        return Response(
                            {"message": f"Payment failed: {status_val}"},
                            status=status.HTTP_400_BAD_REQUEST,
                        )
                logger.error("PayPal API error")
                return Response(
                    {"message": "PayPal API error"}, status=status.HTTP_400_BAD_REQUEST
                )
            except Exception as e:
        ...
        if session_id:
            ...
            try:
                ...
                if payment_status == "captured":
                    with transaction.atomic():
                        ...
                        if locked_order.payment_status == "paid":
                            ...
                            return Response(
                                {"message": "already_paid"}, status=status.HTTP_200_OK
                            )
                        locked_order.payment_status = "paid"
                        locked_order.order_status = "Confirmed"
                    ...
                    return Response(
                        {"message": "processing"}, status=status.HTTP_202_ACCEPTED
                    )
                elif payment_status in ["failed", "cancelled"]:
                    logger.warning(
                        f"Razorpay payment {payment_status} for order {order_id}"
                    )
                    return Response(
                        {"message": "cancelled"}, status=status.HTTP_400_BAD_REQUEST
                    )
                ...
                return Response(
                    {"message": "Razorpay processing error"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if event_type == "PAYMENT.CAPTURE.COMPLETED":
            ...
            if status == "COMPLETED":
                try:
                    with transaction.atomic():
                        order = CartOrder.objects.select_for_update().get(
                        ...
                        if order.payment_status != "paid":
                            ...
                            if order.buyer and order.coupons.exists():
                                for coupon in order.coupons.all():
                                    ...
                                    logger.info(
                                        f"Marked coupon {coupon.code} as used by user {order.buyer.id}"
                                    )

                            order.save()
                            order_items = CartOrderItem.objects.filter(order=order)
                            deactivate_cart(
                                order.oid, order.buyer_id if order.buyer else None
                            )
                            deduct_stock(order_items)
...
class WalletPaymentView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if wallet.balance < order_total:
            logger.info(
                f"Insufficient wallet balance. Required: {order_total}, Available: {wallet.balance}"
            )
            return Response(
                {
        ...
        with transaction.atomic():
            ...
            deactivate_cart(
                locked_order.oid, locked_order.buyer_id if locked_order.buyer else None
            )
            deduct_stock(order_items)
