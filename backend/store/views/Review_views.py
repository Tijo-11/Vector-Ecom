...
from store.models import CartOrder, Product, Review
# Serializers
...
class ReviewListAPIView(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Purchase check
        has_purchased = (
            CartOrder.objects.filter(buyer=user, payment_status="paid")
            .filter(orderitem__product=product)
            .exists()
        )
...
):  # Changed to ListAPIView (no need for Create)
...
class HasPurchasedView(APIView):
    ...
    def get(self, request, product_id):
        ...
        has_purchased = (
            CartOrder.objects.filter(buyer=user, payment_status="paid")
            .filter(orderitem__product__id=product_id)
            .exists()
        )
