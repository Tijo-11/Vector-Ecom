...
def get_effective_discount(product):
    ...
    if product_offers.exists():
        max_product_discount = max(
            offer.discount_percentage for offer in product_offers
        )
    ...
    if product.category:
        ...
        if category_offers.exists():
            max_category_discount = max(
                offer.discount_percentage for offer in category_offers
            )
