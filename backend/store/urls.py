...
from .views.cart_views import (CartAPIView, CartDetailView,
                               CartItemDeleteAPIView, CartListView,
                               CartMergeAPIView)
from .views.checkout_views import (PaymentSuccessView, RazorpayCheckoutView,
                                   WalletPaymentView)
from .views.order_management_views import GuestOrderTrackingView
from .views.order_views import CODOrderConfirmView  # <-- Added COD view
from .views.order_views import (CheckoutView, CouponAPIView, CreateOrderView,
                                OrdersDetailAPIView, RemoveCouponAPIView)
from .views.product_category import (CategoryListView, FeaturedProductListView,
                                     ProductDetailView, ProductListView)
from .views.referral_views import (ApplyReferralView, GenerateReferralView,
                                   MyReferralCouponsView)
from .views.Review_views import (HasPurchasedView, ReviewDetailAPIView,
                                 ReviewListAPIView, SearchProductView)
...
urlpatterns = [
    ...
    path(
        "checkout/<str:order_oid>/", CheckoutView.as_view(), name="checkout"
    ),  # <-- Uses CheckoutView from order_views
    path("coupon/", CouponAPIView.as_view(), name="coupon"),
    # Checkout views (Razorpay & Payment Success)
    path(
        "razorpay-checkout/<str:order_id>/",
        RazorpayCheckoutView.as_view(),
    ...
    path(
        "product/<int:product_id>/has-purchased/",
        HasPurchasedView.as_view(),
        name="has-purchased",
    ),
    ...
    path(
        "guest-track-order/", GuestOrderTrackingView.as_view(), name="guest-track-order"
    ),
    # Referral System
    path(
        "referral/generate/", GenerateReferralView.as_view(), name="generate-referral"
    ),
    path("referral/apply/", ApplyReferralView.as_view(), name="apply-referral"),
    path(
        "referral/my-coupons/",
        MyReferralCouponsView.as_view(),
        name="my-referral-coupons",
    ),
    # Remove coupon
