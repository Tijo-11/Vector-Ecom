...
from .order import (  # , Cart, CancelOrder, Coupon, CouponUsers, DeliveryCountries
...
class OrderCancellation(models.Model):
    ...
    def restore_stock(self):
        ...
        items_to_restore = (
            self.items.all() if not self.is_full_order else self.order.orderitem.all()
        )
        log.info(
            f"Restoring stock for cancellation {self.id}. is_full_order={self.is_full_order}"
        )
        ...
        for item in items_to_restore:
            ...
            product.save()
            log.info(
                f"Restored stock for product {product.title} (ID: {product.id}): {old_stock} + {item.qty} = {product.stock_qty}"
            )
...
class OrderReturn(models.Model):
    ...
    order_item = models.OneToOneField(
        CartOrderItem, on_delete=models.CASCADE, related_name="orderreturn"
    )
    ...
    status = models.CharField(
        max_length=50, choices=RETURN_STATUS_CHOICES, default="pending"
    )
