...
class UpdateOrderStatusView(APIView):
    ...
    def put(self, request, pk):
        ...
        return Response(
            {"message": f"Status updated to '{new_status}'", "data": serializer.data},
...
class HandleReturnRequestView(APIView):
    ...
    def post(self, request, pk):
        ...
        if action == "approve":
            ...
            )  # Refund the item subtotal (excludes shipping)
            ...
            if user:
                ...
                log.info(
                    f"Refund of ₹{refund_amount} credited to user {user.email}'s wallet"
                )
                message = f"Return request approved. ₹{refund_amount} refunded to customer's wallet. Stock has been restored."
            else:
                log.warning(
                    f"Could not credit refund - user not found for order item {order_item.id}"
                )
...
class VendorPendingReturnsCountView(APIView):
    ...
    def get(self, request, vendor_id):
        pending_count = OrderReturn.objects.filter(
            order_item__vendor__id=vendor_id, status="pending"
        ).count()
