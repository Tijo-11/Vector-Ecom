...
# Models
from store.models import CartOrder, CartOrderItem, Product, Review
# Serializers
from store.serializers import (CartOrderItemSerializer, CartOrderSerializer,
                               EarningSummarySerializer, ProductSerializer,
                               ReviewSerializer, SummarySerializer)
...
class FilterOrderAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if filter == "paid":
            ...
            orders = CartOrder.objects.filter(
                vendor=vendor, payment_status="pending"
            ).order_by("-id")
        elif filter == "processing":
            orders = CartOrder.objects.filter(
                vendor=vendor, payment_status="processing"
            ).order_by("-id")
        elif filter == "cancelled":
            orders = CartOrder.objects.filter(
                vendor=vendor, payment_status="cancelled"
            ).order_by("-id")
        elif filter == "latest":
            ...
            orders = CartOrder.objects.filter(
                vendor=vendor, payment_status="paid", order_status="Fulfilled"
            ).order_by("-id")
        elif filter == "Cancelled":
            orders = CartOrder.objects.filter(
            ...
            orders = CartOrder.objects.filter(
                vendor=vendor, payment_status="paid", order_status="Pending"
            ).order_by("-id")
        else:
            orders = CartOrder.objects.filter(vendor=vendor).order_by("id")
        return orders
