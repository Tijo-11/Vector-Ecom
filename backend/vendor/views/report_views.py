...
class SalesReportAPIView(APIView):
    ...
    def get(self, request, vendor_id):
        ...
        queryset = CartOrderItem.objects.filter(
        ...
        if start_date and end_date:
            queryset = queryset.filter(
                order__date__date__gte=start_date, order__date__date__lte=end_date
            )
        ...
        aggregates = queryset.aggregate(
            ...
            total_discount=Sum(
                "saved"
            ),  # assumes 'saved' is discount + coupon deduction per item
            total_items=Sum("qty"),
        )

        items = queryset.values(
            "order__date__date", "product__title", "qty", "sub_total", "saved"
        )
...
class SalesReportPDFView(APIView):
    ...
    def get(self, request, vendor_id):
        ...
        elements.append(
            Paragraph(f"Period: {response_data['period']}", styles["Normal"])
        )
        elements.append(Paragraph("<br/>Summary:<br/>", styles["Normal"]))
        elements.append(
            Paragraph(
                f"Orders: {response_data['summary']['order_count']}<br/>"
                f"Items Sold: {response_data['summary']['sales_count']}<br/>"
                f"Total Amount: ₹{response_data['summary']['order_amount']}<br/>"
                f"Total Discount/Coupons: ₹{response_data['summary']['total_discount']}",
                styles["Normal"],
            )
        )
...
class SalesReportExcelView(APIView):
    ...
    def get(self, request, vendor_id):
        ...
        ws.append(
            ["Total Discount/Coupons (₹)", response_data["summary"]["total_discount"]]
        )
        ws.append([])
