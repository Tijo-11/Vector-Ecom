...
def _build_vendor_transactions(
    vendor, type_filter=None, start_date=None, end_date=None
):
    """Build the transactions list for a vendor, optionally filtered by type and date range."""
    transactions = []

    vendor_order_items = (
        CartOrderItem.objects.filter(vendor=vendor)
        .select_related("order", "order__buyer", "product")
        .order_by("-date")
    )

    vendor_orders = (
        CartOrder.objects.filter(orderitem__vendor=vendor)
        .distinct()
            ...
            transactions.append(
                {
                    ...
                    "user": {
                        ...
                        "full_name": (
                            order.buyer.full_name if order.buyer else order.full_name
                        ),
                    },
                    "related_order": {
                        "oid": order.oid,
            ...
            transactions.append(
                {
                    ...
                    "related_order": (
                        {
                            ...
                            "total": (
                                str(txn.related_order.total)
                                if txn.related_order
                                else None
                            ),
                            "order_status": (
                                txn.related_order.order_status
                                if txn.related_order
                                else None
                            ),
                            "payment_status": (
                                txn.related_order.payment_status
                                if txn.related_order
                                else None
                            ),
                            "payment_method": (
                                txn.related_order.payment_method
                                if txn.related_order
                                else None
                            ),
                        }
                    ...
                    "payment_method": (
                        txn.related_order.payment_method if txn.related_order else None
                    ),
                    "order_type": "refund",
                }
            )
...
class VendorTransactionsListView(APIView):
    ...
    def get(self, request, vendor_id):
        ...
        transactions = _build_vendor_transactions(
            vendor, type_filter, start_date, end_date
        )
        ...
        return Response(
            {
                ...
                "summary": {
                    ...
                    "payment_count": sum(
                        1 for t in transactions if t["transaction_type"] == "payment"
                    ),
                    "refund_count": sum(
                        1 for t in transactions if t["transaction_type"] == "refund"
                    ),
                    "pending_count": sum(
                        1 for t in transactions if t["transaction_type"] == "pending"
                    ),
                },
    ...
    def get(self, request, transaction_id):
        ...
        if transaction.related_order_item:
            data["related_order_item"] = {
                ...
                "product_title": (
                    transaction.related_order_item.product.title
                    if transaction.related_order_item.product
                    else None
                ),
...
class VendorWalletStatsView(APIView):
    ...
    def get(self, request, vendor_id):
        ...
        vendor_orders = CartOrder.objects.filter(orderitem__vendor=vendor).distinct()
        ...
        vendor_item_ids = CartOrderItem.objects.filter(vendor=vendor).values_list(
            "id", flat=True
        )
        vendor_order_ids = vendor_orders.values_list("id", flat=True)
...
class WalletReportPDFView(APIView):
    ...
    def get(self, request, vendor_id):
        ...
        start_date, end_date, period = _get_date_range(request)
        transactions = _build_vendor_transactions(
            vendor, start_date=start_date, end_date=end_date
        )
