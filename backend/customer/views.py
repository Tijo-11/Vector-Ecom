...
from store.serializers import (CartOrderSerializer, NotificationSerializer,
                               WishlistSerializer)
# Models
...
class OrdersAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        logger.info(
            f"User ID: {self.request.user.id if self.request.user.is_authenticated else 'None'}"
        )
        logger.info(
            f"Auth header: {self.request.META.get('HTTP_AUTHORIZATION', 'No auth header')}"
        )
        ...
        orders = CartOrder.objects.filter(
            ...
            & (
                ...
                | Q(payment_status="processing")
                | Q(payment_status="cancelled")
            )
        )
...
class VerifyPaymentView(APIView):
    ...
    def post(self, request, user_id):
        ...
        if not all([razorpay_payment_id, razorpay_order_id, razorpay_signature]):
            return Response(
                {"error": "Missing payment data"}, status=status.HTTP_400_BAD_REQUEST
            )
        try:
...
class CustomerWalletTransactionsView(APIView):
    ...
    def get(self, request, user_id):
        ...
        transactions = (
            ...
            .select_related("related_order", "related_order_item")
            .order_by("-created_at")
        )
