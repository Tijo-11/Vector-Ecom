...
from .views import ChangePasswordView  # New logged-in change password
from .views import PasswordChangeView  # This is the forgot-password flow
from .views import \
    ProfileView  # Old profile view (kept for backward compatibility if needed)
from .views import (AddressViewSet, ChangeEmailRequestView,
                    MyTokenObtainPairView, PasswordEmailVerify, RegisterView,
                    UserProfileDetailView, UserProfileUpdateView,
                    VerifyEmailChangeView, VerifyEmailOTP, google_login)
...
urlpatterns = [
    ...
    path(
        ...
        PasswordEmailVerify.as_view(),
        name="Pasword_Reset",
    ),
    path(
        "user/password-change/", PasswordChangeView.as_view(), name="password_reset"
    ),  # Forgot password flow
    path(
        "user/profile/<user_id>/", ProfileView.as_view(), name="Profile_view"
    ),  # Kept for any legacy frontend calls
    ...
    path(
        "user/profile/", UserProfileDetailView.as_view(), name="user-profile-detail"
    ...
    path(
        "user/change-password/", ChangePasswordView.as_view(), name="change-password"
