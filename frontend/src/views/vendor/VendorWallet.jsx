...
function VendorWallet() {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const response = await apiInstance.get(`vendor/wallet-transactions/${vendorId}/`, { params });
  ...
  const handleDownload = async (format) => {
      ...
      try {
        const res = await apiInstance.get(endpoint, { params, responseType: "blob" });
  ...
  const chartData = useMemo(() => {
    ...
    transactions.forEach(t => {
       ...
       if (!grouped[dateStr]) {
         grouped[dateStr] = { payment: 0, refund: 0, date: new Date(t.created_at) };
       }
        ...
        <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
          ...
          <div className="p-6 space-y-5">
            ...
            {transaction.related_order && (
              <div className="bg-blue-50 rounded-xl p-4">
                ...
                <div className="space-y-2 text-sm">
                   <div className="flex justify-between"><span className="text-blue-600">Order ID</span><span className="font-medium text-blue-900">{transaction.related_order.oid}</span></div>
                   <div className="flex justify-between"><span className="text-blue-600">Total</span><span className="font-medium text-blue-900">₹{parseFloat(transaction.related_order.total).toLocaleString('en-IN')}</span></div>
                   <div className="flex justify-between"><span className="text-blue-600">Status</span><span className="font-medium text-blue-900 capitalize">{transaction.related_order.order_status}</span></div>
                </div>
              </div>
            )}
          ...
          <div className="p-6 border-t border-gray-100 space-y-3">
             {isRefundOrCancel && transaction.related_order?.oid && (
              <Link to={`/vendor/orders/${transaction.related_order.oid}/`} className="w-full bg-blue-600 text-white font-semibold py-3 rounded-xl hover:bg-blue-700 transition flex items-center justify-center gap-2">
                <ExternalLink size={18} /> View Order
              </Link>
             )}
            <button onClick={onClose} className="w-full bg-gray-100 text-gray-700 font-semibold py-3 rounded-xl hover:bg-gray-200 transition">Close</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="flex bg-gray-50 min-h-screen">
      ...
      <div className="flex-1 p-8 lg:p-12 overflow-x-hidden">
         ...
         <div className="mb-8">
            ...
            <p className="text-gray-500 mt-1">Manage your earnings, view transaction history and download reports.</p>
         </div>
            ...
            <div className="lg:col-span-3 bg-gradient-to-r from-gray-900 to-gray-800 rounded-2xl shadow-xl p-8 text-white relative overflow-hidden">
                <div className="relative z-10 flex flex-col justify-between h-full">
                    ...
                    <div className="mt-8 pt-6 border-t border-gray-700 flex flex-wrap gap-6">
                        <div><p className="text-xs text-gray-400">Total Earned</p><p className="font-semibold text-lg">₹{stats?.total_earned?.toFixed(2) || "0.00"}</p></div>
                        <div><p className="text-xs text-gray-400">Pending Payouts</p><p className="font-semibold text-lg">₹{stats?.pending_payouts?.toFixed(2) || "0.00"}</p></div>
                        <div><p className="text-xs text-gray-400">Total Refunded</p><p className="font-semibold text-lg text-orange-400">₹{stats?.total_refunded?.toFixed(2) || "0.00"}</p></div>
                    </div>
                </div>
         ...
         <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4">
               ...
               <div className="flex flex-wrap items-center gap-3">
                  ...
                  {period === "custom" && (
                    <>
                       <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none" />
                       <span className="text-gray-400">-</span>
                       <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm outline-none" />
                    </>
                  )}
                  
                  <button onClick={fetchTransactions} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
               ...
               {/* Right: Downloads */}
               <div className="flex items-center gap-3">
                  <button onClick={() => handleDownload('pdf')} className="flex items-center gap-2 px-4 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
                     <FileText size={16} className="text-red-500" /> PDF
                  </button>
                  <button onClick={() => handleDownload('excel')} className="flex items-center gap-2 px-4 py-2 border border-gray-200 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
         ...
         <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-8">
             <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                 <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                    ...
                    {period === 'custom' ? 'Filtered Transactions' : `Transaction History (${period})`}
                 </h2>
