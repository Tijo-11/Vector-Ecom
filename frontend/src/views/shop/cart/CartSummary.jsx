...
function CartSummary({ cartItems, setCartTotal }) {
  ...
  useEffect(() => {
    const fetchCartTotal = async () => {
      if (!cart_id || cart_id === "undefined") {
        const emptyTotals = { mrp_total: 0, discounted_total: 0, shipping: 0, grand_total: 0 };
      ...
      try {
        ...
        setCartTotal({ itemCount, ...serverTotals });
         ...
         const fallback = { mrp_total: 0, discounted_total: 0, shipping: 0, grand_total: 0 };
  ...
  const shippingDisplay = cart_total.shipping > 0 ? `₹${cart_total.shipping.toFixed(2)}` : "Free";
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
      ...
      <div className="space-y-4 text-sm">
        <div className="flex justify-between text-gray-600">
          ...
          <span className="font-medium text-gray-900">₹{cart_total.mrp_total.toFixed(2)}</span>
        </div>
        
        {cart_total.mrp_total > cart_total.discounted_total && (
             <div className="flex justify-between text-green-600">
                <span>Total Discount</span>
                <span className="font-medium">-₹{(cart_total.mrp_total - cart_total.discounted_total).toFixed(2)}</span>
             </div>
        )}
        
        <div className="flex justify-between text-gray-600 items-center">
          <span>Shipping</span>
          <span className="font-medium text-gray-900 bg-gray-100 px-2 py-1 rounded">{shippingDisplay}</span>
        </div>

        <div className="border-t border-dashed border-gray-200 pt-4 mt-4">
           <div className="flex justify-between items-end">
              <span className="text-base font-bold text-gray-900">Total</span>
              <span className="text-2xl font-bold text-blue-600">₹{cart_total.grand_total.toFixed(2)}</span>
           </div>
      ...
      {hasInsufficientStock ? (
          <div className="mt-6 bg-red-50 border border-red-100 rounded-lg p-3 text-center">
             <p className="text-sm text-red-600 font-medium animate-pulse">
                Some items exceed available stock.<br/>Adjusting automatically...
             </p>
          </div>
      ) : (
          !isAddressPage && (
