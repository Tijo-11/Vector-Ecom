...
export default function ProductDetail() {
  ...
  const fetchWishlist = async () => {
    ...
    try {
      ...
      const wishlistItems = Array.isArray(response.data) ? response.data : response.data.results || [];
    ...
    apiInstance
      ...
      .then((response) => {
        ...
        if (response.data?.category?.slug) {
             apiInstance.get(`products/?category=${response.data.category.slug}`)
                .then(res => {
                    const allProds = res.data.results || [];
                    const filtered = allProds.filter(p => p.id !== response.data.id).slice(0, 4);
      ...
      <div className="bg-gray-50 border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
           <div className="flex items-center text-sm text-gray-500">
              ...
              {product.category && (
                <>
                 <Link to={`/category/${product.category.slug}`} className="hover:text-blue-600">{product.category.title}</Link>
                 <ChevronRight className="h-4 w-4 mx-2" />
                </>
              )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    {allImages.map((img, idx) => (
                       <button
                         ...
                         className={`w-20 h-20 flex-shrink-0 rounded-lg overflow-hidden border-2 transition-all ${
                            mainImage === img.image ? "border-blue-600 ring-2 ring-blue-100" : "border-transparent opacity-70 hover:opacity-100"
                         }`}
                       >
           ...
           <div>
              ...
              <div className="flex items-center gap-4 mb-6">
                 ...
                 <span className={`text-sm font-medium px-2 py-0.5 rounded ${isOutOfStock ? "bg-red-100 text-red-600" : "bg-green-100 text-green-600"}`}>
              ...
              {product.description && (
                  <p className="text-gray-600 mb-8 line-clamp-3 leading-relaxed">
                     ...
                     <button onClick={() => setActiveTab('description')} className="text-blue-600 hover:underline text-sm ml-1 font-medium">Read more</button>
                  </p>
              )}
              ...
              <div className="grid grid-cols-3 gap-4 mt-8 pt-8 border-t border-gray-100">
                 <div className="flex flex-col items-center text-center">
                    ...
                    <span className="text-xs text-gray-500 font-medium">Fast Shipping</span>
                 </div>
                 <div className="flex flex-col items-center text-center">
                    <ShieldCheck className="h-6 w-6 text-gray-400 mb-2" />
                    <span className="text-xs text-gray-500 font-medium">Authentic</span>
                 </div>
                 <div className="flex flex-col items-center text-center">
                            ...
                            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
                               {product.specification.map((spec, idx) => (
                                  <div key={idx} className="flex border-b border-gray-200 pb-2">
                                     <dt className="font-semibold text-gray-900 w-1/3">{spec.title}</dt>
                                     <dd className="text-gray-600 w-2/3">{spec.content}</dd>
                                  </div>
                               ))}
                            </dl>
                         </div>
