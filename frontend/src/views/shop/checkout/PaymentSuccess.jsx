...
function PaymentSuccess() {
  ...
  const displayPaymentMethod =
    paymentMethodQuery === "COD"
      ? "Cash on Delivery"
      : paymentMethodQuery === "Wallet"
        ? "Wallet"
        : razorpayPaymentId
          ? "Online (Razorpay)"
          : paypalCaptureId
  ...
  return (
    <div className="bg-gray-100 min-h-screen py-10 print:bg-white print:py-0">
      <div
        id="invoice-section"
        className="max-w-4xl mx-auto bg-white shadow-xl rounded-2xl overflow-hidden print:shadow-none"
      >
        {/* Success Header */}
          ...
          <div className="mb-10">
            ...
            <div className="border border-t-0 border-gray-100 rounded-b-lg">
              {order.orderitem?.map((item, i) => (
                <div
                  key={i}
                  className="grid grid-cols-12 p-4 border-b border-gray-50 last:border-0 text-sm"
                >
            ...
            {Number(order.offer_saved ?? 0) + Number(order.coupon_saved ?? 0) >
              ...
              <div className="flex justify-between w-full md:w-1/2 text-green-600">
                ...
                <span>
                  -₹
                  {(
                    Number(order.offer_saved ?? 0) +
                    Number(order.coupon_saved ?? 0)
                  ).toFixed(2)}
                </span>
              </div>
            )}
