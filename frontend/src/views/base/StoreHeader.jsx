...
import { 
  ...
  LogOut, LayoutDashboard, Settings, Heart, Bell, ShoppingBag, Truck, Package
} from "lucide-react";
            ...
            <div className="hidden md:flex flex-1 max-w-lg mx-8">
              <div className="relative w-full group">
                ...
                >
                  <input
                    type="text"
                    className="block w-full pl-10 pr-3 py-2.5 border border-gray-200 rounded-full leading-5 bg-gray-50 text-gray-900 placeholder-gray-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all duration-200 sm:text-sm"
            ...
            {isVendor && (
              <div className="relative" ref={vendorRef}>
                ...
                >
                  ...
                  Vendor
                  <ChevronDown className={`h-4 w-4 transition-transform duration-200 ${isVendorOpen ? 'rotate-180' : ''}`} />
                </button>
                ...
                <div
                  className={`absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg ring-1 ring-black ring-opacity-5 transform transition-all duration-200 origin-top-right ${
                    isVendorOpen ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"
                  }`}
                >
                  <div className="py-1">
                    <Link to="/vendor/dashboard/" className="flex items-center px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 hover:text-indigo-600">
                      <LayoutDashboard className="mr-3 h-4 w-4" /> Dashboard
                    </Link>
                    <Link to="/vendor/products/" className="flex items-center px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 hover:text-indigo-600">
                      <ShoppingBag className="mr-3 h-4 w-4" /> Products
                    </Link>
                    <Link to="/vendor/orders/" className="flex items-center px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 hover:text-indigo-600">
                  ...
                  >
                    ...
                    <div className="py-2">
                      ...
                      <Link to="/customer/wishlist/" className="flex items-center px-5 py-3 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-colors">
                        <Heart className="mr-3 h-4 w-4 text-gray-400" /> Wishlist
                      </Link>
                      <Link to="/customer/notifications/" className="flex items-center px-5 py-3 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-colors">
                        <Bell className="mr-3 h-4 w-4 text-gray-400" /> Notifications
                      </Link>
                      <div className="border-t border-gray-100 my-1"></div>
                      <Link to="/logout" className="flex items-center px-5 py-3 text-sm text-red-600 hover:bg-red-50 transition-colors">
          ...
          <div className="flex-1 overflow-y-auto py-6 px-6 space-y-6">
            ...
            {showPublicFeatures && (
              ...
              >
                <input
                  type="text"
                  className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
            ...
            {isLoggedIn && (
               <div className="flex items-center gap-3 p-4 bg-blue-50 rounded-xl">
                  <div className="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold text-lg">
                    {userProfile?.image ? (
                        <img src={userProfile.image} alt="" className="h-full w-full rounded-full object-cover"/>
                    ) : (
                        (userProfile?.full_name || user?.username || "U")[0].toUpperCase()
                    )}
                  </div>
            ...
            <nav className="space-y-2">
              <Link to="/" className="flex items-center px-4 py-3 text-gray-700 hover:bg-gray-50 hover:text-blue-600 rounded-xl transition-colors font-medium">
                Home
              </Link>
              
              {isVendor && (
                <div className="space-y-1">
                  <p className="px-4 text-xs font-semibold text-gray-400 uppercase tracking-wider mt-4 mb-2">Vendor Area</p>
                  <Link to="/vendor/dashboard/" className="flex items-center px-4 py-2.5 text-gray-700 hover:bg-gray-50 hover:text-indigo-600 rounded-lg transition-colors">
                    <LayoutDashboard className="mr-3 h-5 w-5 text-gray-400" /> Dashboard
                  </Link>
                  <Link to="/vendor/products/" className="flex items-center px-4 py-2.5 text-gray-700 hover:bg-gray-50 hover:text-indigo-600 rounded-lg transition-colors">
                     <ShoppingBag className="mr-3 h-5 w-5 text-gray-400" /> Products
                  </Link>
                </div>
              )}
              ...
              {isLoggedIn && !isAdmin && !isVendor && (
                <div className="space-y-1">
                   ...
                   <Link to="/customer/orders/" className="flex items-center px-4 py-2.5 text-gray-700 hover:bg-gray-50 hover:text-blue-600 rounded-lg transition-colors">
                    <Package className="mr-3 h-5 w-5 text-gray-400" /> Orders
                   </Link>
                   <Link to="/track-order" className="flex items-center px-4 py-2.5 text-gray-700 hover:bg-gray-50 hover:text-blue-600 rounded-lg transition-colors">
